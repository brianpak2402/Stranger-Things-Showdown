...
void setPixel(int row, int col, u16 color) {
  // Set pixel at (row,color) to the input color
...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  // Set our source as the desired color
  DMA[3].src = &color;
  // Iterate through each row and transfer color to each pixel in the row.
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  // Transfer each row in image to the (row + i)th row of videoBuffer
  for (int i = 0; i < height; i++) {
...
void fillScreenDMA(volatile u16 color) {
  // Transfer color to the entire video buffer
  DMA[3].src = &color;
